...
S_BASED = {"s"}                 # знаки, чья осевая строится в обход примитивов

# Чем именно строится осевая такого знака. Функция получает метрику и
# возвращает СПИСОК осевых — знак бывает не из одного пруткa.
#
# Заведено под цифры: у них круглые формы эллиптические (по ширине —
# строчной o, по высоте — выносному вверх), а примитив дуги в шрифте
# только круговой. Прежде обход был один и намертво привязан к s;
# ş пользовалась им же. Теперь обход общий, а s просто стоит в нём первой.
CENTRE = {}


def centrelines(ch, m):
    return CENTRE[ch](m) if ch in CENTRE else [s_centreline(m)]
...
def skeleton(ch, m):
    ...
    if ch in S_BASED:
        rec = list(centrelines(ch, m)) + [list(p) for p in REC]
    else:
