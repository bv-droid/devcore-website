...
def _ink(px, w, h):
    """Плотность краски: 1.0 — полное покрытие, 0.0 — чистая бумага.

    Нормируется на РАЗМАХ бумага-краска, а не на бумагу. Делить на бумагу
    значило привязать замер к тому, насколько краска тёмная: у полного
    пикселя масса выходила (бумага − краска) / бумага, то есть у тёмной
    краски больше, чем у светлой.

    Поймалось это переводом листов на принятую оснастку. Краска стала
    #392B1E вместо #514F4A, масса полного пикселя выросла на пятую часть,
    квант оптического края набирался раньше, край уехал внутрь — и втяжка
    съехала с 66.6 на 66.8 без единой правки геометрии. Оптический край —
    свойство ФОРМЫ, и от цвета краски он зависеть не должен.
    """
    paper, ink = max(px), min(px)
    span = max(1e-6, float(paper - ink))
    return [min(1.0, max(0.0, (paper - v) / span)) for v in px[:w * h]]
