...
Что нашлось глазом, а не сверкой, — и потому здесь заведено
  ...
  Отсюда пять сверок раздела ДОКУМЕНТ: doctype, кодировка, язык, ноль
  внешних запросов и вшитая гарнитура. Проверены отказом на прежнем
  файле из коммита — расходятся все пять. Первый вариант мерки ловил
  четыре: кавычку внутри url('https://…') я не учёл, то есть мерка не
  видела ровно ту строку, из-за которой всё и случилось.
