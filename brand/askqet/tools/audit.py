...
if __name__ == "__main__":
    ...
    if sgj:
        ...
        word = min(sgj["survive"][c]["floor"] for c in "askqet")
        pair = min(sgj["survive"][c]["floor"] for c in "?§")
        check("ПАРА", "держит не хуже слова", "пара умирает на", pair,
              "слово умирает на", word,
              state=("СХОДИТСЯ" if pair >= word else "РАСХОДИТСЯ"))
