...
if __name__ == "__main__":
    ...
    if sgj:
        pr = sgj["pair"]
        check("ПАРА", "воздух между массами", "пара (замер)", pr["gap"],
              "слово (принято)", pr["air"], tol=0.05)
        check("ПАРА", "интерлиньяж не перенесён", "пара", pr["lead"],
              "слова — было бы ошибкой", pr["word_lead"],
              state=("СХОДИТСЯ" if abs(pr["lead"] - pr["word_lead"]) > 1.0
                     else "РАСХОДИТСЯ"),
              note="правило то же, число своё")
        check("ПАРА", "очко вопроса", "чаша знака", sgj["room"]["counter"],
              "круглая форма букв", sgj["room"]["letter"], tol=0.05)
