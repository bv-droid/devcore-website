...
Что проверяется
  ...
  ЛЯССЕ. Свес q — не деталь на конце хвоста, а лента: она уходит вниз на
  80 единиц от базовой и кончается ласточкиным хвостом. Проверяется то же,
  что и раньше: вырез обязан оставаться вырезом, а не заплывать. Разница в
  том, что теперь он это делает дольше самого знака, а не меньше.
...
# Ляссе. Срез — ГЛУБОКИЙ, вырез 1.1 ширины ленты: единственный из шести,
# который не трогает метрики блока. Но сам вырез — только конец; ляссе
# держится на двух числах, и оба выбраны замером в tools/awards.py.
#
#   ШИРИНА 1.5 штриха — по дефекту, который лежал в этой самой проверке и
#   не лечился: вырез умирал на 56 px, а знак жив до 46 — деталь умирала
#   раньше знака. При 1.5 вырез живёт до 36 px.
#   СВЕС 60 единиц — по максимуму отстояния силуэта от СВОЕЙ пустышки.
#   Максимум пришёлся ровно туда, где знак становится квадратным.
#
# Разведены два начертания, и путать их нельзя. SP — НАБОРНОЕ: свес у q
# обычный, иначе в тексте лента въедет в следующую строку. MARK — ЗНАК.
TAIL = 1.1
DROP = 60.0
RIBBON = 1.5
SP = L.style(st=ST, tail=TAIL)
MARK = L.style(st=ST, tail=TAIL, drop=DROP, ribbon=RIBBON)
...
def block(ind, sp=MARK, color=INK):
