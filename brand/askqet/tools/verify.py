...
Что проверяется
  ...
  ВТЯЖКА. По литере — полная ширина a плюс поправка на оптику. q обязана
  вставать под s по ОПТИЧЕСКОМУ краю, не по наборному. Замер идёт краской
  ЗАМЕРА, чёрным по белому: прежде он шёл принятой краской и от неё
  зависел — при смене оснастки втяжка уехала с 66.6 на 66.8 без единой
  правки геометрии.
  ...
  иначе в аватаре его нет. ПОСТАНОВКА выведена в tools/clamps.py: коробка
  ставится от массы строчных, а не от выносных; зазор рамки строчный, а не
  буквенный; плечо кончается у колонки выносных. Все четыре плеча обязаны
  стоять от краски на одном расстоянии — разброс проверяется там же.
...
# Зазор рамки — СТРОЧНЫЙ, а не буквенный: воздух между массами строк.
#
# Стоял буквенный, 0.9 штриха = 11.7, и это была ошибка сразу в двух
# местах (разобрано замером в tools/clamps.py). Просветы внутри слова —
# 16.0, 16.0, 14.1, 5.9; зазор 11.7 лежал ровно посреди этого ряда, и
# правая стойка рядом со штрихом t читалась не рамкой, а ещё одной буквой.
# Рамка — уровень строки, а не буквы, и зазор ей нужен строчный.
GAP = AIR

# Плечо больше не доля габарита. Доля была назначена (0.44) и оказалась
# почти ровно средним двух выведенных — среднее и стёрло связь с буквами.
# Теперь плечо идёт до КОЛОНКИ ВЫНОСНЫХ и встаёт за зазор до неё.
ARM = 0.44                     # прежняя доля; осталась для мелких знаков,
#                                у которых выносных нет и колонки не из чего
#                                строить — см. icon.py
# Стойка уголка — доля той длины, на которой она доходила бы до базовой
# своей строки. Горизонтальное плечо так укоротить нельзя: его длину
# задаёт колонка выносных, и она выведена. У стойки такой опоры нет, и
# полная длина была не выводом, а обычаем.
#
# Перебор шести долей (tools/clamps.py): зазор везде остаётся 22.0, то
# есть столкновения не возникает ни при какой длине. Отличие силуэта с
# укорочением падает — 0.336 при полной, 0.312 при половинной, — а
# отстояние от пустышки, наоборот, растёт и держит максимум в середине
# перебора: 0.449 · 0.457 · 0.463 · 0.461.
#
# Заказчик выбрал 0.50, и на нём вылезло то, чего не поймал ни мой первый
# перебор, ни моя же рекомендация 0.66: укоротившись, НИЗ-ПРАВО СТОЙКА
# перестаёт стоять рядом с t, и ближайшая краска уходит наискось. Зазоры
# по плечам:
#
#   1.00 · 0.82   22.0  22.0  22.0  22.0   разброс 0.00
#   0.66          22.0  22.0  22.0  23.8   разброс 1.84
#   0.50          22.0  22.6  22.0  35.1   разброс 13.06
#
# То есть 0.50 возвращает разброс почти туда, с чего начинали (13.78) —
# и отменяет ровно ту починку, ради которой уголки и разбирались.
#
# Принято 0.58 — середина между лёгкостью и свойством, и выбрана она уже
# ПОСЛЕ того, как цена была названа. Разброс 6.58 не дефект, а объявленная
# цена решения: сводная сверка держит эту строку открытой и печатает её
# каждым прогоном, чтобы она не забылась.
#
# Предел без цены — 0.82. Если когда-нибудь свойство окажется важнее
# лёгкости, возвращаться сюда, к одному числу.
VERT = 0.58
VERT_FREE = 0.82               # предел, на котором разброс ещё нулевой
...
# Краска ЗАМЕРА. Плашки, по которым считаются просветы и вырез, рисуются
# чёрным по белому, а не принятой краской: замер формы от цвета зависеть
# не должен. Поймано на втяжке — там из-за этого уехало число, см.
# hanging.M_INK. Здесь порог берётся серединой размаха и потому устоял,
# но держаться на везении незачем.
...
def inner(thick):
    """Поле внутри уголка: сам уголок плюс зазор рамки."""
    return thick + GAP
...
def frame(ind, thick=THICK, sp=SP):
    ...
    return dict(x0=x0, y0=y0, x1=x1, y1=y1, thick=thick, g=g,
                ...
                ay_t=((g["base1"] + GAP) - y0) * VERT,
                ay_b=(y1 - (g["xtop2"] - GAP)) * VERT)


def frame_simple(w0, h0, thick=THICK):
    """Рамка для содержимого БЕЗ колонки выносных — мелкие знаки.

    Плечо здесь остаётся долей габарита: выводить его не из чего, колонки
    нет. У мелкого знака нет двух строк, и вынос там либо один, либо его
    нет совсем — останавливать плечо не у чего. Это ОБЪЯВЛЕННОЕ исключение
    из правила рамки, а не забытый случай: зазор и охранное поле у него те
    же, что у знака, разная только длина плеча.
    """
    p = thick + GAP
    W, Hh = w0 + p * 2, h0 + p * 2
    return dict(x0=0.0, y0=0.0, x1=W, y1=Hh, thick=thick, pad=p,
                ax_t=W * ARM, ay_t=Hh * ARM, ax_b=W * ARM, ay_b=Hh * ARM,
                g=dict(x0=p, y0=p, x1=p + w0, y1=p + h0))


def clamp_rects(F):
    """Четыре прямоугольника пары уголков в координатах коробки."""
    x0, y0, x1, y1, t = F["x0"], F["y0"], F["x1"], F["y1"], F["thick"]
    return [(x0, y0, x0 + F["ax_t"], y0 + t),
            (x0, y0, x0 + t, y0 + F["ay_t"]),
            (x1 - F["ax_b"], y1 - t, x1, y1),
            (x1 - t, y1 - F["ay_b"], x1, y1)]


def frame_box(F):
    """Габарит нарисованного знака: рамка вместе с буквами."""
    g = F["g"]
    return (min(F["x0"], g["x0"]), min(F["y0"], g["y0"]),
            max(F["x1"], g["x1"]), max(F["y1"], g["y1"]))
...
def check_counters(ind):
    ...
    body, w0, h0 = block(ind, color=M_INK)
    src = svg(f'  <rect width="{n(w0)}" height="{n(h0)}" fill="{M_PAPER}"/>\n'
              f'  {body}\n', box=(w0, h0), title="")
...
if __name__ == "__main__":
    ...
    items = [
        dict(key="mark", title="ЗНАК", means=f"уголок {pick['t']:.2f} штриха",
             ...
             note=f"Втяжка по литере {ind:.1f}, интерлиньяж {LEAD:.0f}, ляссе "
                  ...
                  f"{inner(thick):.1f} — уголок плюс зазор рамки {GAP:.0f}; тоньше поле "
    ...
    rows = [
        ...
        ("охранное поле", f"{inner(thick):.1f}", f"уголок плюс зазор рамки {GAP:.0f}"),
    ]
    for a, b, c in rows:
        print(f"  {a:<24}{b:>10}   {c}")
