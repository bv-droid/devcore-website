...
A6 = 336.0                     # где начинается верхний терминал шестёрки


def c_6(m):
    """Шестёрка: очко внизу и дуга, уходящая ВПРАВО ВВЕРХ.

    Длина терминала выбрана перебором на ПОЧИНЕННОЙ мерке спутывания
    (tools/confusion.py: доля разошедшейся краски, а не доля кадра).
    Размен тут чистый и виден насквозь — чем длиннее терминал, тем дальше
    шестёрка от b и тем ближе к девятке:

        288°   6/b 0.164   6/9 0.587
        312°   6/b 0.205   6/9 0.491
        336°   6/b 0.258   6/9 0.368   ← принято
        348°   6/b 0.278   6/9 0.323   очко 2 — БРАК

    На 348° дуга смыкается с чашей и запирает лишний просвет: очков
    становится два вместо одного. Это не «чуть хуже», это другая буква.
    Взято 336: 6/b поднялась с 0.181 до 0.258, а до девятки остаётся
    0.368 — заметно дальше, чем до b.

    У b стойка прямая и слева, весь правый верх пуст; у шестёрки верхний
    терминал уходит вправо и занимает этот угол. Больше их различать нечем.
    """
    f = fm(m)
    r = f["rs"] * 1.34
    lo = -f["st"] / 2 - r + f["ov"]
    top = ell(f["cx"], -f["H"] + f["ry"], f["rx"], f["ry"], A6, 182.0)
...
def confusion(sp=SP):
    """Пары, которые в справочнике путают чаще всего.

    Мерка своя здесь больше не считается: она жила в этом модуле, делила
    расхождение на КАДР и потому занижала всё вчетверо. Починенная лежит
    в tools/confusion.py — там расхождение делится на краску пары, и там
    же сказано, почему порога нет и не будет. Держать в системе две мерки
    одного и того же значит однажды поверить не той: старая показывала
    6/b как 0.089, новая — 0.26.

    Ввозится внутри функции, а не сверху: confusion.py сам импортирует
    цифры отсюда, и наверху вышло бы кольцо.
    """
    import confusion as CF
    M = CF.masks(sorted({c for pair in CONFUSE for c in pair}), sp)
    return [dict(pair=f"{a}/{b}", diff=CF.diff(M[a], M[b]))
            for a, b in CONFUSE]


def _confusion_old(sp=SP):
    """Прежняя мерка. Оставлена мёртвой — на неё ссылается запись выше."""
...
if __name__ == "__main__":
    ...
    print("\nСПУТАТЬ — доля разошедшейся краски (tools/confusion.py)\n")
    print("порога нет: он не откалибровался, и назначать его значило бы "
          "судить\nбуквы собственным мнением. Это ПОРЯДОК — чинить сверху "
          "вниз.\n")
    print(f"{'пара':>8}{'различие':>11}")
    for c in sorted(C, key=lambda c: c["diff"]):
        print(f"{c['pair']:>8}{c['diff']:>11.3f}")
    bad = []
    ...
    print(f"\nхуже всех {sorted(C, key=lambda c: c['diff'])[0]['pair']} — "
          f"её и чинить следующей.")
