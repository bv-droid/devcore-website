...
GAP = inner(THICK) - THICK     # 11.7 — задуманный зазор от уголка до краски
...
def build_now(g):
    """Как стоит сейчас: коробка по габариту, плечи — доля габарита."""
    G = inner(THICK)
    x0, y0 = g["x0"] - G, g["y0"] - G
    x1, y1 = g["x1"] + G, g["y1"] + G
    W, Hh = x1 - x0, y1 - y0
...
if __name__ == "__main__":
    ...
    print(f"задуманный зазор {GAP:.1f} = девять десятых штриха. "
