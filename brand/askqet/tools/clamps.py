...
VERT = (1.0, 0.82, 0.66, 0.50, 0.36, 0.24)   # доли принятой стойки


def build_vert(g, k):
    """Принятое построение с УКОРОЧЕННОЙ стойкой.

    Горизонтальное плечо не трогается: его длину задаёт колонка выносных,
    и она выведена. Вертикальное такой опоры не имеет — оно доходит до
    базовой своей строки, и это скорее обычай, чем вывод. Значит его и
    надо перебрать: короче уголок — легче знак, но в какой-то момент он
    перестаёт быть уголком и становится засечкой.
    """
    B = build_air(g)
    R = B["R"]
    x0, y0, x1, y1, t = B["x0"], B["y0"], B["x1"], B["y1"], THICK
    av = (R["tl_v"][3] - R["tl_v"][1]) * k
    bv = (R["br_v"][3] - R["br_v"][1]) * k
    B = dict(B)
    B["R"] = dict(R, tl_v=(x0, y0, x0 + t, y0 + av),
                  br_v=(x1 - t, y1 - bv, x1, y1))
    return B
