...
def fig_tail(ind, P):
    """Ляссе крупно: лента целиком, от базовой до ласточкина хвоста."""
    sp = MARK
...
def build():
    ...
    # Пределы берутся из свежего прогона проверки, а не переписываются сюда
    # руками. Когда ляссе стало лентой, вырез стал жить до 34 px вместо 56 —
    # и вписанное числом «до 56 px» осталось бы враньём, которого никто не
    # заметит. Документ обязан ломаться вместе со знаком, а не переживать его.
    VER = load("verify")
    COL = load("color")
    ...
    spec = [
        ...
        ("вырез ляссе", f"{TAIL:.1f} ширины ленты", "угол среза держится "
         "при любой ширине"),
        ("лента", f"{DESC + DROP:.0f} вниз от базовой",
         f"ширина {RIBBON:.1f} штриха — по замеру, а не по вкусу"),
    ...
    limits = [
        ...
        ("ляссе", f"до {VER['tail']['alive']:.0f} px",
         "ниже два зубца выреза сливаются в один — теперь вырез переживает "
         "сам знак, а не умирает раньше него"),
    ...
    open_items = [
        ("прописные", "Строчные построены целиком — латиница и казахский "
         "набор, tools/alphabet.py. Прописных нет: знак строчной, и до "
         "заголовков они не понадобились."),
        ("чистота знака", "Проверку на занятость товарного знака делает "
         "патентный поверенный. Это не моя часть, и я её не выдумываю."),
...
<section>
  ...
  <div class="body">
    <h2>Два лок-апа</h2>
    <p class="lede">Слово набрано в две строки с втяжкой по литере. Свес
    <em>q</em> уходит вниз лентой и кончается ласточкиным хвостом —
    вплетённая в корешок закладка. Блок прихвачен двумя уголками по
    диагонали.</p>
...
<section>
  ...
  <div class="body">
    <h2>Лента, а не вырез</h2>
    <p class="lede">Сначала ляссе было срезом на конце хвоста, и это
    оказалось ошибкой. Разборка знака показала её числом: если снять вырез
    целиком, силуэт в тридцати двух пикселях меняется на
    <strong>0.001</strong>. Единственная собственная мысль знака в нём не
    читалась — один процент площади. Держали знак уголки, а они есть у
    кого угодно.</p>
    <p>Поэтому свес <em>q</em> доведён до настоящей длины: он уходит вниз
    на {DESC + DROP:.0f} единиц от базовой и кончается тем же ласточкиным
    хвостом. Это не украшение поверх буквы — это её собственный выносной
    элемент. Хвост <em>q</em> и был единственной частью знака, которая
    свисает под строку и у которой конец свободен; изменилось не место, а
    масштаб. Цветом лента занимает теперь
    <strong>{COL['tail_share'] * 100:.1f} %</strong> площади знака вместо
    одного.</p>
    <p>Оба числа ленты взяты замером, а не на глаз. Ширина {RIBBON:.1f}
    штриха — наименьшая, при которой вырез живёт до
    {VER['tail']['alive']:.0f} px: до этого он умирал на 56 px, то есть
    раньше самого знака, и этот дефект лежал в проверке невылеченным.
    Длину не смогла выбрать ни одна мерка, пока пустышка для сравнения
    была одна на всех — высокая форма отходит от квадратной плашки просто
    потому, что высокая. Своя пустышка каждому кандидату — и у мерки
    появился максимум внутри перебора. Он пришёлся ровно туда, где знак
    становится квадратным.</p>
    <p>Глубина выреза {TAIL:.1f} ширины ленты — не штриха: угол среза
    держится, насколько бы лента ни расширилась книзу. Из шести
    испробованных срезов — ласточкин хвост, острый конец, косой срез,
    ступень, продольный прорез, поперечный перегиб — принят первый: он
    снимает больше всех краски и дольше всех живёт на мелком.</p>
