...
def fig_pair(C, w=170):
    """Второй вариант знака. Строится ТЕМ ЖЕ mark(), что и слово."""
    import signs as SG
    body, w0, h0 = SG.pair_mark(color=C["word"], corner=C["corner"])
    k = w / w0
...
<section>
  ...
  <div class="body">
    ...
    два знака препинания одно под другим: наверху то, с чем приходит
    предприниматель, внизу то, чем ему отвечает право. Вопрос и параграф.</p>
    <figure>{fig_pair(C)}
    <figcaption>Второй вариант. Втяжка {SGN["pair"]["indent"]:.2f},
    интерлиньяж {SGN["pair"]["lead"]:.1f}, уголки {THICK:.1f} — те же, что
    у слова.</figcaption></figure>
    <p><strong>Оба знака нарисованы, а не набраны.</strong> Соблазн был
    очевидный — взять «?» и «§» у {esc(fam)} и поставить в рамку. Тогда
    второй вариант марки был бы набран чужим шрифтом, стоя рядом с первым,
    набранным своим: два почерка в одной марке видно сразу. Оба построены
    тем же скелетом и той же машиной, что буквы слова.</p>
    <p><strong>У вопроса нет отдельной стойки, и это следствие замера.</strong>
    Чаша берёт круглую форму букв — очко {SGN["room"]["counter"]:.1f}, ровно
    как у строчной o. При таком очке чаша доходит почти до точки; чтобы
    выкроить стойку хотя бы в штрих, очко пришлось бы ужать до семнадцати.
    Очко решает, как знак живёт на мелком, стойка только рисует — выбрано
    очко. Низ чаши и есть низ стойки.</p>
    <p><strong>У параграфа своя растяжка дуг</strong> — {SGN["wide"]:.2f}
    против {SGN["s_wide"]:.2f} у самой s. При растяжке s две дуги внахлёст
    смыкаются в кляксу: щель зарастала на пятом шаге растекания, тогда как
    слово держит семь — пара умирала бы раньше слова, ради мелкого размера
    и заведённая. У s внахлёст ничего не стоит, у § стоит вторая s, и
    переносить растяжку вслепую было ошибкой.</p>
    ...
    <p>Интерлиньяж пары {SGN["pair"]["lead"]:.1f}, а не {LEAD:.0f}, как у
    слова: обе фигуры ростом с выносное, и при семидесяти четырёх просвет
    между ними вышел бы полторы единицы. Перенесено <em>правило</em> —
    воздух {SGN["pair"]["air"]:.0f} между массами, — а не число.</p>
    <p><strong>Ляссе у пары нет, и это не забывчивость.</strong> Ляссе —
    вырез на прямом свесе: у q свес идёт вниз стойкой, и её конец срезан
    ласточкиным хвостом. У параграфа прямого свеса нет вовсе, низ у него
    круглый. Пересадить вырез можно было бы, только перестав рисовать
    параграф. Пару держат уголки — они общие у обоих вариантов.</p>
  </div>
</section>
