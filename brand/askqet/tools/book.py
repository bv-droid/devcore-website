...
def build():
    ...
    forbidden = [
        ...
        ("живые маски в продакшене", "Вырез ляссе держится маской. В рабочие "
         "файлы контуры должны быть запечены."),
    ]
    forb_html = "".join(
    ...
    open_items = [
        ("рабочие файлы", "Контуры не запечены: вырез ляссе живёт маской, "
         "а уголки — отдельными путями. Для типографии и вышивки нужен "
         "комплект с плоскими контурами."),
        ("алфавит", "Построены шесть литер — a, s, k, q, e, t. Для "
         "заголовков и казахской латиницы нужен полный набор."),
