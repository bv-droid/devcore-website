...
def fig_pair(C, w=200):
    """Второй вариант знака: «?§» в строку, без рамки."""
    import signs as SG
    body, w0, h0 = SG.pair_mark(color=C["word"])
    k = w / w0
    return (f'<svg viewBox="0 0 {n(w0)} {n(h0)}" width="{n(w)}" '
            f'height="{n(h0 * k)}" role="img" '
            f'aria-label="Знак AskQet: вопрос и параграф">{body}</svg>')
...
def build():
    ...
    # ── Пара: живучесть обоих вариантов рядом ───────────────────────────
    def _why(v):
        if v["die"]:
            return "очко гибнет"
        return "щель зарастает" if v["seal"] else "не умирает в переборе"

    surv_html = "".join(
        f'<tr><td class="glyphs">{esc(ch)}</td>'
        f'<td class="num">{SGN["survive"][ch]["holes"]}</td>'
        f'<td class="num">{SGN["survive"][ch]["floor"]}</td>'
        f'<td>{_why(SGN["survive"][ch])}</td></tr>'
        for ch in "askqet?§")
    ...
    open_items = [
        ...
        ("ляссе у пары", "Второй вариант знака идёт без ляссе: вырез "
         "бывает на прямом свесе, а у параграфа свес круглый. Ось spine "
         "проходит, но менять ей нечего. Понадобится подпись и во втором "
         "варианте — придётся рисовать параграфу другой низ, то есть "
         "рисовать другой параграф. Пока пару держат уголки."),
...
<header class="cover">
  ...
  <dl>
    ...
    <dt>знак</dt><dd>два варианта: слово и пара «?/§»</dd>
...
<section>
  <div class="aside">Пара</div>
  <div class="body">
    <h2>Второй знак: вопрос и параграф</h2>
    <p class="lede">Знак живёт в двух вариантах. Первый — слово. Второй —
    два знака препинания рядом: то, с чем приходит предприниматель, и то,
    чем ему отвечает право. Вопрос и параграф.</p>
    <figure>{fig_pair(C)}
    <figcaption>Принятое исполнение: в строку, без рамки. Габарит
    {SGN["pair"]["w"]:.0f}&nbsp;×&nbsp;{SGN["pair"]["h"]:.0f}, отношение
    сторон {SGN["pair"]["ratio"]:.2f} — почти квадрат.</figcaption></figure>
    <p><strong>В строку, а не столбиком.</strong> Столбиком пара повторяла
    бы складку слова, но два знака — не два слова: в строку они дают
    квадрат, а квадрат и нужен там, ради чего пара заводилась, — аватар,
    печать, фавикон, корешок. Отступ между знаками не назначен: это
    обычные боковые поля шрифта, те же, что между буквами слова.</p>
    <p><strong>Без рамки.</strong> Уголки держат первый вариант; второй
    держится начертанием. Ляссе во второй вариант тоже не переносится —
    это вырез на прямом свесе, а у параграфа свес круглый.</p>
    <p><strong>Оба знака нарисованы, а не набраны.</strong> Соблазн был
    очевидный — взять «?» и «§» у {esc(fam)}. Тогда второй вариант марки
    был бы набран чужим шрифтом, стоя рядом с первым, набранным своим:
    два почерка в одной марке видно сразу. Оба построены тем же скелетом
    и той же машиной, что буквы слова.</p>
    <p><strong>Вопрос со стойкой, и это стоило очка.</strong> Выбор был
    настоящий: либо чаша берёт круглую форму букв — очко 27.6, как у
    <em class="ch">o</em>, — и тогда она доходит почти до точки, стойки
    нет, а знак читается завитком; либо стойка есть, и очко ужимается.
    Выбрана стойка: вопрос должен читаться вопросом.</p>
    <p>Раз выбор сделан, доля не назначена, а выведена. Условий два, оба
    уже приняты в шрифте: стойка не короче штриха и просвет над точкой в
    три четверти штриха. Отсюда чаша {SGN["room"]["bowl"]:.1f} — предел,
    при котором стойка ещё остаётся, — и очко
    {SGN["room"]["counter"]:.2f} против буквенных
    {SGN["room"]["letter"]:.2f}.</p>
    <p><strong>Цена напечатана, а не спрятана.</strong> Мерка щели её не
    ловила: апертура у вопроса широкая, и по ней знак не умирает за весь
    перебор. Поэтому чаша замерена прямо — точкой в её середине: она
    заплывает краской на {SGN["pair"]["clog"]["?"]}-м шаге растекания
    против {SGN["pair"]["clog"]["o"]}-го у строчной o. Цена стойки —
    {SGN["pair"]["clog"]["o"] - SGN["pair"]["clog"]["?"]}&nbsp;шага. Слово
    при этом умирает на седьмом, так что вопрос по-прежнему не слабое
    звено.</p>
    <p><strong>У параграфа своя растяжка дуг</strong> — {SGN["wide"]:.2f}
    против {SGN["s_wide"]:.2f} у самой s. При растяжке s две дуги внахлёст
    смыкаются в кляксу: щель зарастала на пятом шаге, тогда как слово
    держит семь — пара умирала бы раньше слова, ради мелкого размера и
    заведённая. У s внахлёст ничего не стоит, у § стоит вторая s.</p>
    <table><thead><tr><th>знак</th><th class="num">очков</th>
    <th class="num">умирает на шаге</th><th>чем умирает</th>
    </tr></thead><tbody>{surv_html}</tbody></table>
    <p>Отвергнутое исполнение — столбиком, с рамкой — осталось записью в
    <code>tools/signs.py</code>: втяжка по оси круглых форм и интерлиньяж,
    выведенный из воздуха между массами. Понадобится вертикальная складка
    — начинать не с нуля.</p>
  </div>
</section>
